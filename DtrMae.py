            ...
            if feature_datatypes[column] == "number":
                ...
                if len(set(feature_values_sorted)) <= 200:
                    for set_value in  list(set(feature_values_sorted)):
                        ...
                        mae1 = self.mae(targs1)
                        mae2 = self.mae(targs2)
                        #print("coninuous cat")
                        if mae1 is None or mae2 is None:
                            continue
                            mae_total = mae1 + mae2
                        if (lowest_mae is None) or np.all(mae_total < lowest_mae):
                            lowest_mae = mae_total
                ...
                for feature_value in set(feature_values):
                    ...
                    # Check mae value also
        if lowest_mae is not None and \
